...
- **Description:** OpenEI U.S. Utility Rate Database price context for New Hampshire service territory. The current implementation fetches approved default residential rates near Manchester, NH.
- **Requires:** Public access is attempted first. `OPENEI_API_KEY` is optional and can be set if OpenEI rejects unauthenticated requests.
- **Format:** JSON -> CSV
...
- **Requires:** No key for development (`DEMO_KEY` default). Optional NREL API key for higher limits at https://developer.nrel.gov/signup/
- **Format:** JSON -> CSV
...
- **Description:** EPA eGRID subregion output emission-rate context. The current implementation ingests the public summary table with CO2, CO2e, NOx, and SO2 output emission rates.
- **Format:** HTML -> CSV
